...
def load_nodes(task_plan: Path) -> list[str]:
    if not task_plan.exists():
        return ["题目任务", "数据与指标", "建模思路", "核心模型", "模型求解", "验证分析", "结论输出"]
...
def svg(nodes: list[str], width: int = 1200) -> str:
    ...
    parts = [
        ...
        '<style>text{font-family:Arial,"PingFang SC","Microsoft YaHei",sans-serif;font-size:18px;fill:#0f172a}.box{fill:#f8fafc;stroke:#2563eb;stroke-width:2;rx:8}.arrow{stroke:#334155;stroke-width:2;fill:none;marker-end:url(#arrow)}</style>',
    ...
    for i, label in enumerate(nodes):
        ...
        parts.append(f'<rect class="box" x="{x}" y="{y}" width="{box_w}" height="{box_h}"/>')
        safe = html.escape(label)
        parts.append(f'<text x="{x + box_w/2}" y="{y + box_h/2 + 6}" text-anchor="middle">{safe}</text>')
    for (x1, y1), (x2, y2) in zip(centers, centers[1:]):
        parts.append(f'<path class="arrow" d="M{x1 + box_w/2 - 6},{y1} L{x2 - box_w/2 + 6},{y2}"/>')
    parts.append("</svg>")
    return "\n".join(parts)
