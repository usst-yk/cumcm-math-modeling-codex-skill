...
如果已经有论文初稿，需要补一张可放入正文的技术路线图：

```text
[$cumcm-math-modeling] 请基于下面这篇 CUMCM 论文生成技术路线图。要求逻辑清晰、内容简洁、黑白风格，不要装饰性小图或图标，不要 AI 味表达。请输出论文技术路线图和所采用模型的流程框图，使用 Mermaid flowchart，并附可直接放入论文的图注和说明文字。
```
...
```text
[$cumcm-math-modeling] 请基于论文中采用的 XX 模型生成模型流程框图。要求从输入数据、数据预处理、变量构造、模型假设、目标函数或模型方程、参数估计或算法求解、结果输出、验证分析到结论解释依次展开；黑白风格，节点文字简洁，并给出论文图注。
```
