...
```text
[$cumcm-math-modeling] 只做这道题的第一问。请先判断第一问的评分点和隐含约束，再给 3 条建模路线并比较，选择主路线；然后完整写出第一问的变量定义、目标函数、约束条件、解析推导、算法步骤、验证方案、可视化设计和可直接放进论文的中文段落。
```
...
```text
[$cumcm-math-modeling] 下面是我对第一问的现有解法。请按 CUMCM 一等奖标准重做：指出当前解法浅在哪里，补充机制分析或解析推导，加入基准模型、主模型、稳健性检验、灵敏度分析和论文写法。
```
